...
import com.transporte.guias.dto.ErrorResponse;
...
public class GlobalExceptionHandler {
	...
	/**
	 * Maneja excepciones cuando el objeto ya existe en S3
	 */
	@ExceptionHandler(S3ObjectAlreadyExistsException.class)
	public ResponseEntity<ErrorResponse> handleS3ObjectAlreadyExistsException(S3ObjectAlreadyExistsException ex,
			WebRequest request) {

		log.error("Objeto ya existe: {}", ex.getMessage());

		ErrorResponse error = ErrorResponse.builder().timestamp(LocalDateTime.now())
				.status(HttpStatus.CONFLICT.value()).error("Object Already Exists").message(ex.getMessage())
	...
	public ResponseEntity<ErrorResponse> handleInvalidRequestException(InvalidRequestException ex, WebRequest request) {
		...
		ErrorResponse error = ErrorResponse.builder().timestamp(LocalDateTime.now())
				.status(HttpStatus.BAD_REQUEST.value()).error("Invalid Request").message(ex.getMessage())
				.path(getPath(request)).build();
	...
	public ResponseEntity<ErrorResponse> handleGuiaNotFoundException(GuiaNotFoundException ex,
			WebRequest request) {

		log.error("Estudiante no encontrado: {}", ex.getMessage());

		ErrorResponse error = ErrorResponse.builder().timestamp(LocalDateTime.now())
				.status(HttpStatus.NOT_FOUND.value()).error("Student Not Found").message(ex.getMessage())
				.path(getPath(request)).build();
